...
export function Navbar({ todos, filter, setFilter }: NavbarProps) {
    ...
    return (
        <div className="flex absolute bottom-4">
            <div>{todos.filter(todo => !todo.completed).length} осталось задач</div>
            <div className="ml-4 space-x-4">
                <button
                    className={filter === TodoListFilter.All ? 'border-solid border-2 border-sky-500' : ''}
                    onClick={() => { setFilter(TodoListFilter.All) }}>all</button>
                <button
                    className={filter === TodoListFilter.Active ? 'border-solid border-2 border-sky-500' : ''}
                    onClick={() => { setFilter(TodoListFilter.Active) }}>active</button>
                <button
                    className={filter === TodoListFilter.Completed ? 'border-solid border-2 border-sky-500' : ''}
